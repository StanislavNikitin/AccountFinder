...
import ru.innopolis.finder.web.httpclient.cookie.CookieProvider;
import ru.innopolis.finder.web.httpclient.cookie.CookieStore;
import ru.innopolis.finder.web.httpclient.headers.HttpHeader;
import ru.innopolis.finder.web.httpclient.headers.HttpHeaders;
import ru.innopolis.finder.web.httpclient.headers.KeepAliveStrategy;
...
public class HttpBrowser {
    ...
    private CookieStore cookieStore; //хранилище cookie
     ...
     */
    public HttpBrowser(String uri, boolean storeFormParams){
        ...
        try {
            cookieStore = new CookieStore(storePath);
        } catch (IOException e){
        ...
        Registry<org.apache.http.cookie.CookieSpecProvider> r = RegistryBuilder.<org.apache.http.cookie.CookieSpecProvider>create()
        .register(CookieSpecs.DEFAULT, new DefaultCookieSpecProvider(publicSuffixMatcher))
        .register(CookieProvider.PROVIDER_NAME, new CookieProvider())
        .build();
        ...
        httpClient = HttpClients.custom()                                       //кастомный httpClient
                .setKeepAliveStrategy(new KeepAliveStrategy(keepAliveTime))   //установка дефолтного keep-alive time в значение keepAliveTime
                .setUserAgent(userAgent)                                        //установка дефолтного user-agent'а 
                .setDefaultHeaders(HttpHeaders.getDefaultHeadersList())       //установка заголовков, которые будут добавляться всегда
                .setDefaultRequestConfig(RequestConfig.custom()                 //установка дефолтного request-config
                        .setCookieSpec(CookieProvider.PROVIDER_NAME)      //..установка кастомного куки процессора
                        .build())                                               //..
     ...
     * @return HttpResponse - пустой или нет, зависит от успешности выполнения запроса.
     */
    private HttpResponse executeRequest(boolean reqType){
        ...
        try {
            ...
            if (this.httpResponse != null){
                ...
                List<HttpHeader> resultResponseHeaders = new ArrayList<HttpHeader>();
                ...
                for (Header currHeader : responseHeaders){ 
                    ...
                    resultResponseHeaders.add(new HttpHeader(currHeaderName, parseHeaderParameters(it)));
                }
                ...
                result = new HttpResponse(responseCode, resultResponseHeaders, null, 0, finalLocation); //инициализируем response без content и content-length
                ...
                if (responseEntity != null){ //если есть контент
                    ...
                    try {
                        ...
                        try {
                            ...
                            result.setResponseContet(EntityUtils.toString(responseEntity)); //если и это не сработало - устанавливаем без кодировки
                        }
                        
                    } 
                    result.setContentLength(contentLength); //устанавливаем также длину контента
                }
                
            } else throw new Exception("Can't execute request");
        } catch (Exception e){
     ...
     * @return 
     */
    private String getResponseContentCharset(HttpResponse response){

        String charset = defaultCharset;
        if (response == null) return charset;
        HttpHeader contentTypeHeader = response.getHeaderByName("Content-Type");  //получаем инфу о заголовке Content-Type
     ...
     * @return null, если URI некорректно или равно null; "пустой" HttpResponse, если не удалось выполнить запрос; не пустой HttpResponse, если всё успешно.
     */
     ...
     * @return null, если URI некорректно или равно null; "пустой" HttpResponse, если не удалось выполнить запрос; не пустой HttpResponse, если всё успешно.
     */
    public HttpResponse sendGetRequest(String host){
     ...
     * @return null, если URI некорректно или равно null; "пустой" HttpResponse, если не удалось выполнить запрос; не пустой HttpResponse, если всё успешно.
     */
    public HttpResponse sendPostRequest(String host){
